				...
				<div class="dialog dialog-shadow" style="display: block; margin-top: -362px;">
					...
					<div v-if="loginPage" class="content">
						<ul class="common-form">
							...
							<li>
								<div class="input">
									<el-input type="password" v-model="ruleForm.password" @keyup.enter="login" placeholder="密码"></el-input>
								</div>
							</li>
							<li style="text-align: right" class="pr">
								...
								<a href="javascript:;" class="register" @click="toRegister">注册 agnes 账号</a>
								<a style="padding: 1px 0 0 10px" @click="open('找回密码','请联系作者邮箱找回密码或使用测试账号登录：test | test')">忘记密码 ?</a>
							</li>
						</ul>
						<!--登陆-->
						<div style="margin-top: 25px">
							<y-button :text="logintxt" :classStyle="ruleForm.password&& ruleForm.username&& logintxt === '登录'?'main-btn':'disabled-btn'" @btnClick="login" style="margin: 0;width: 100%;height: 48px;font-size: 18px;line-height: 48px"></y-button>
						</div>
						<!--返回-->
						<div>
							<y-button text="返回" @btnClick="login_back" style="marginTop: 10px;marginBottom: 15px;width: 100%;height: 48px;font-size: 18px;line-height: 48px">
							</y-button>
						</div>
						...
						<div class="footer">
							...
							<a>
								<img @click="open('待开发','此功能开发中...')" style="height: 15px; margin-top: 22px;" src="/static/images/other-login.png">
							</a>
						</div>
					</div>
				</div>
		...
		methods: {
			...
			login() {
				...
				userLogin(params).then(res => {
					if(res.code === 1) {
						setStore('userId',res.user.id)
						setStore('token',res.user.token)
						this.messageSuccess(res.msg + "，正在跳转...")
//						alert(res.user)
						this.$router.push({
							path: "/"
						})
//						this.$router.go(0)
		...
		mounted() {
			this.getRemembered()
			this.login_addCart()
			this.notify('登录提示', '账号密码：admin | admin')
		},
		components: {
			YButton
		}
